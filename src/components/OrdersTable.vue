<template>
<div>
    <b-table small hover :items="items"
    :fields="fields"
    >
        <template #cell(status)="data">
        {{data.item.status}}<br>
        <b-button size="sm" v-b-modal.order-status-form variant="primary" @click="editStatus(data.item.id, data.item.status)" ><b-icon icon="pencil-square" aria-hidden="true"></b-icon> Изменить</b-button>
        ...
        <template #cell(address)="data">
            <template v-if="data.item.address != null">
                ...
                строение {{data.item.address.numberOfBuild}}, подъезд {{data.item.address.numberOfEntrance}}
            </template>
            <template v-else>
                Неизвестно
            </template>
